...
class Filer(models.Model):
    ...
    def links(self):
        ...
        for q in qs_links:
            ...
            if qs_names.count() > 0:
                name_obj = qs_names[0]
                name = (name_obj.namt + ' ' + name_obj.namf + ' ' + name_obj.naml + ' ' + name_obj.nams).strip()
                try:
                    description = LookupCode.objects.get(code_id=q.link_type).code_desc
                except:
                    description = 'failed to get a description for lookup code / "link_type" %s' % q.link_type
                d[q.filer_id_b] = {
...
class Gift(models.Model): # Activity Expenses in the parlance of the forms
    '''
        This model is the catchall for "activity expenses" reported on the various forms.
    ...
    back_reference_id = models.CharField(max_length=20L) # Back reference to the parent transaction id
    ...
    memo_code = models.CharField(max_length=1L, null=True, blank=True) # Field name made lowercase.
    memo_refno = models.CharField(max_length=20L, null=True, blank=True) # Field name made lowercase.
